...
The GUI runs on the Arduino UNO Q's Linux side (MPU), and drives its onboard RGB LED and 12x8 LED matrix as a physical status display for the RIID system. `mcu_interface.py`'s `ArduinoInterface` is a thin RPC client, over the [`Arduino_RouterBridge`](https://github.com/arduino-libraries/Arduino_RouterBridge) Unix socket (`/var/run/arduino-router.sock`), to the sketch running on the board's MCU core — see [`mcu/README.md`](../mcu/README.md) for the firmware side and its RPC method table. An unreachable bridge (the MCU sketch not flashed yet, or the GUI running off-board during development) is handled gracefully rather than failing: `ArduinoInterface.get_status()` reports whether the connection is up, and every call site checks it before pushing an update, so the rest of the GUI still works if the display side isn't provisioned yet.
...
- `_execute_ml_pipeline()` re-drives the LED matrix text with the detected isotope(s) (or `"Background"`), but only while a survey is running (`state == 'RIID_SURVEY'`) — since that method is only ever called from inside the survey loop, it stops firing the instant the survey stops, so it can never race with or overwrite `set_state()`'s status text.