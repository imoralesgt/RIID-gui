...
Isotope classification runs fully on-device via [LiteRT](https://ai.google.dev/edge/litert) (TFLite), with no network calls. Compiled models live in `gui/ml_models/`, are loaded by `gui/ml_inference.py`, and are the models actually served by the GUI at runtime — independent from `ml-core/`, is deprecated.
